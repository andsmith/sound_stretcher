...
class Spectrogram(object):
    ...
    def __init__(self, bbox, sound, resolution_hz, resolution_sec, freq_range=None):
        ...
        freq_range = (0, sound.metadata.framerate /
                      2) if freq_range is None else freq_range

        z, f, self._t_bin_centers, self._stft_params = get_power_spectrum(self._data,
                                                                          self._f_rate,
                                                                          freq_range=freq_range,
                                                                          resolution_hz=self._res_hz,
                                                                          resolution_sec=self._res_sec)

        self._z_orig = z  # save for editing & inverting
    ...
    def clear_spectrum_region(self, bbox, t, zoom_t, zoom_f, pan_f):
        """
        Calculate the region of the spectrogram to clear, and clear it.
        Invert the resulting spectrum back into a waveform and return it.
        :param bbox:  bounding box of the region to clear (xmin, ymin, xmax, ymax)
        ...
        :return:  new_sound_seg:  np.array of the new sound segment
        """
        (f_low_i, f_high_i), (t_low_i, t_high_i), _ = self._get_window_limits(
            t, zoom_t, zoom_f, pan_f)

        # Get box bounds in Hz and seconds
        clear_f_low, clear_f_high = np.sum(
            self._f <= bbox[1]), np.sum(self._f <= bbox[3])
        clear_t_low, clear_t_high = np.sum(
            self._t_bin_centers < bbox[0]), np.sum(self._t_bin_centers < bbox[2])

        # Zero out the region of the spectrum user wants to clear, & the mirrored region of the
        # spectrum, to keep the result real-valued.
        new_coefficients = self._z_orig[:, t_low_i:t_high_i]

        new_coefficients[clear_f_low:clear_f_high,
                         clear_t_low: clear_t_high] = 0
        new_coefficients[-clear_f_high:-clear_f_low,
                         clear_t_low: clear_t_high] = 0

        # Get the interval of time we're editing, find the indices in the original data.
        t_low, t_high = self._t_bin_centers[t_low_i], self._t_bin_centers[t_high_i]

        sound_ind_start = int(t_low * self._f_rate)
        sound_ind_end = int(t_high * self._f_rate)
        # invert the spectrum back to a real-valued waveform
        inv_params = self._stft_params.copy()
        del inv_params['padded']
        print( new_coefficients.shape)
        new_sound_seg = istft(new_coefficients, **inv_params)


        import ipdb
        ipdb.set_trace()

    def _get_window_limits(self, t, zoom_t, zoom_f, pan_f):
        """
        Get the indices of the spectrogram window, and the exact cursor location (t).

        :param t:  time index (float) in [0, sound_duration_sec]
        :param zoom_t:  float in (0, inf), stretched_time_t =  normal_time_t / zoom_t (smaller is zoomed in more)
        :param zoom_f:  float in (0, 1], how much of the spectrum to see (vertically)
        :param pan_f:   float in (0, 1], which portion of the spectrum to see
        :return:  f_low_i, f_high_i, t_low_i, t_high_i (indices into self._f, self._t_bin_centers, self._power)
        """

        # get F indices for spectrogram image
        # offset wrt un-displayed portion of the interval [0, 1]
        pan_amount = (1.0 - zoom_f) * pan_f
        # interval, both endpoints in [0, 1]
        f_range_rel = np.array((pan_amount, pan_amount + zoom_f))
        # interval, both endpoints in Hz
        freq_range = self._freq_range[1] * f_range_rel
        # index into f (y-axis of spectrogram)
        ...
        # pin default zoom to window size with this
        n_time_samples = int(np.ceil(self._width * zoom_t))
        ...
        if t_high_i == t_low_i:
            ...
            if t_high_i > self._t_bin_centers.size - 1:
                t_low_i -= 1
                t_high_i -= 1
        return (f_low_i, f_high_i), (t_low_i, t_high_i), t_ind
    ...
    def draw(self, frame, t, zoom_t, zoom_f, pan_f, contrast, cursor=False, axes=True):
        ...
        """
        (f_low_i, f_high_i), (t_low_i, t_high_i), t_ind = self._get_window_limits(
            t, zoom_t, zoom_f, pan_f)
        ...
        spectrogram_img = cv2.applyColorMap(
            spectrogram_img_mono, cv2.COLORMAP_HOT)
        ...
        if cursor:
            ...
            cursor_x = int(t_relative * self._width +
                           self._bbox['left'] - self._cursor_width / 2)
            draw_v_line(spectrogram_img_resized, cursor_x,
                        self._cursor_width, self._cursor_color[:3])
        # put in frame
